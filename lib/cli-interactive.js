...
function incorrectWordChoices(word, message, filename, options, done) {
  const suggestions = options.suggestions ? spellcheck.suggest(word) : [];
...
function getCorrectWord(word, filename, options, done) {
  inquirer
    ...
    .then(async function(answer) {
      ...
      // TODO: refactor this code to take advantage of asynchronous checking
      for (let i = 0; i < newWords.length; i++) {
        const newWord = newWords[i];
        if (
          filters.filter([{ word: newWord }], options).length > 0 &&
          !(await spellcheck.checkWord(newWord))
        ) {
          hasMistake = true;
        }
      }
