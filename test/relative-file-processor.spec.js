...
describe('relative file processor', () => {
  ...
  it('should work with empty patterns', async () => {
    ...
    await fileProcessor.relativeFileProcessor([], {}, fileCallSpy, finishedSpy);

    expect(fileCallSpy).not.toHaveBeenCalled();
    expect(finishedSpy).toBeCalledTimes(1);
    expect(spellConfig.initialise).not.toHaveBeenCalled();
  });
  ...
  it('should work with single pattern', async () => {
    ...
    await fileProcessor.relativeFileProcessor(
      ['1'],
      {},
      fileCallSpy,
      finishedSpy
    ...
    expect(fileCallSpy).toBeCalledTimes(1);
    expect(finishedSpy).toBeCalledTimes(1);
